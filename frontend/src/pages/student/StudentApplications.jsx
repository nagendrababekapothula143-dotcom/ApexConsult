...
const StudentApplications = () => {
  ...
  const handlePlacementUpdate = async (id, status) => {
    try {
      setUpdatingId(id);
      const res = await api.patch(`/applications/${id}/placement`, { placementStatus: status });
      if (res.data.success) {
        await fetchData(['applications']);
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {loading ? (
          ...
          applications.map((app) => (
            <div key={app._id} className="bg-white border border-slate-200/70 rounded-3xl p-6 shadow-sm flex justify-between items-start gap-4 hover:border-indigo-200 hover:shadow-md transition-all group flex-col">
                  ...
                  <div className="flex flex-col gap-2 items-end">
                    ...
                    {app.placementStatus && (
                      <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-extrabold uppercase tracking-wider ${app.placementStatus === 'placed' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                        {app.placementStatus === 'placed' ? 'Got the Job 🎉' : 'Not Placed'}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex justify-between items-end mt-5">
                  {app.resumeUrl ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-xs text-indigo-600 hover:text-indigo-700 font-bold inline-flex items-center gap-1.5 no-underline bg-indigo-50 px-3 py-1.5 rounded-lg w-fit transition-colors"
                  ...
                  {!app.placementStatus && (
                    <div className="flex gap-2">
                      <button 
                        onClick={() => handlePlacementUpdate(app._id, 'placed')}
                        disabled={updatingId === app._id}
                        className="text-[10px] font-bold px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-colors disabled:opacity-50"
                      >
                        Got the Job 🎉
                      </button>
                      <button 
                        onClick={() => handlePlacementUpdate(app._id, 'not_placed')}
                        disabled={updatingId === app._id}
                        className="text-[10px] font-bold px-3 py-1.5 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 transition-colors disabled:opacity-50"
                      >
                        Not Placed
                      </button>
                    </div>
