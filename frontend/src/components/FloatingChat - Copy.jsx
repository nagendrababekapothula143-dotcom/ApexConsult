...
const FloatingChat = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      setAdminTyping(res.data.adminTyping);

      const ticketRes = await api.get('/tickets/student');
          ...
          <div className="p-3 bg-white border-t border-slate-100 shrink-0">
            {activeTicket?.status === 'Resolved' ? (
              <div className="text-center py-2">
                <p className="text-xs text-slate-500 mb-2">This conversation has been resolved.</p>
                <button 
                  onClick={() => {
                    setActiveTicket(null);
                    setMessages([]);
                  }}
                  className="bg-indigo-50 text-indigo-700 px-4 py-2 rounded-full text-xs font-bold hover:bg-indigo-100 transition-colors w-full"
                >
