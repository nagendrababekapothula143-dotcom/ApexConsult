...
router.get('/', protect, async (req, res) => {
  try {
    ...
    if (ticketId) {
      // If filtering by ticketId, scan and filter (since no GSI on ticketId)
      // For production, we'd add an index on ticketId, but scan is fine for MVP
      command = new ScanCommand({
        TableName: 'consulting_messages',
        FilterExpression: 'ticketId = :ticketId AND studentId = :studentId',
...
router.post('/', protect, async (req, res) => {
  try {
    const { text, targetStudentId, ticketId } = req.body;
...
router.get('/admin', protect, authorize('admin'), async (req, res) => {
  try {
    const { studentId, ticketId } = req.query;
